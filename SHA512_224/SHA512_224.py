# Default State
def Default_State(): 
    return [0x8C3D37C819544DA2,0x73E1996689DCD4D6,0x1DFAB7AE32FF9C82,0x679DD514582F9FCF,0x0F6D2B697BD44DA8,0x77E36F7304C48942,0x3F9D85A86A1D36C8,0x1112E6AD91D692A1]

# SHA-512 padding implementation
def pad(message,l): 
    final = []
    p = message + b"\x80" + b"\x00" * ((128 - l - 1 - 16) % 128) + (l*8).to_bytes(16,byteorder="big")
...
def sha512_224(message:bytes):

    # Default State
    H = Default_State()

    # Pad the message to 128 length length blocks
    ...
    # Return H0||H1||H2||H3[:32]
    return (b"".join([f.to_bytes(8,"big") for f in H]))[:28].hex()
