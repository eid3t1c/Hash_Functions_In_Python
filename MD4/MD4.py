...
# MD4 padding implementation
def pad(message,l): 
    final = []
    p = message + b"\x80" + b"\x00" * ((64 - l - 1 - 8) % 64) + (l*8).to_bytes(8,byteorder="little")
...
def MD4(message:bytes):
    # Default MD4 state
    Registers = Default_State() 

    # Pad and seperate Blocks
    padded_message = pad(message,len(message))

    # How many blocks
    blocks = len(padded_message)
